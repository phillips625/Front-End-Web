...
//In this example, Modernizr checks if geolocation is supported by the browserand enabled by the user.
var elMap = document.getElementById('loc'); // HTML element
//When getCurrentPos i t ion() is called, the user will be asked for permission to share their location.
...
// If it is not supported, then the user will see a message that shows theirlocationcould not be found.
else {  // Not supported
	elMap.textContent = msg; // Add manual entry
}

//If the location is gained, the user's latitude and longitude arewritteninto the page.
...
// Ifthelocationisnotgained(foranyreason),again the message will say that a location cannot be found. Theerror code is logged to the browserconsole.
